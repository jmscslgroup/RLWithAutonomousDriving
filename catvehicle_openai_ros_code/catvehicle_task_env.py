...
from openai_ros.robot_envs import MyRobotEnv #from John's catvehicle
...
register(
        id='CatVehiceMaze-v0', # change name
        entry_point='openai_ros.task_envs.turtlebot2.turtlebot2_maze:TurtleBot2MazeEnv', ## change later!!!
        max_episode_steps=timestep_limit_per_episode,
    )

class CatVehicleMazeEnv(johns-code_catvehicle_env.MyRobotEnv): # change location
    def __init__(self):
        """
        ...
        number_actions = rospy.get_param('/catvehicle2/n_actions') #change the namespace in yaml file
        ...
        self.dec_obs = rospy.get_param("/catvehicle2/number_decimals_precision_obs", 1)
        self.linear_forward_speed = rospy.get_param('/catvehicle2/linear_forward_speed')
        self.linear_turn_speed = rospy.get_param('/catvehicle2/linear_turn_speed')
        self.angular_speed = rospy.get_param('/catvehicle2/angular_speed')
        self.init_linear_forward_speed = rospy.get_param('/catvehicle2/init_linear_forward_speed')
        self.init_linear_turn_speed = rospy.get_param('/catvehicle2/init_linear_turn_speed')
        
        
        self.n_observations = rospy.get_param('/catvehicle2/n_observations')
        self.min_range = rospy.get_param('/catvehicle2/min_range')
        self.max_laser_value = rospy.get_param('/catvehicle2/max_laser_value')
        self.min_laser_value = rospy.get_param('/catvehicle2/min_laser_value')
        
        # Here we will add any init functions prior to starting the MyRobotEnv
        super(CatVehicleMazeEnv, self).__init__() #change to catvehicle name
        ...
        rospy.logdebug("get_dist len===>"+str(get_dist)
        ...
        # Rewards
        self.forwards_reward = rospy.get_param("/catvehicle2/forwards_reward")  # change the namespace to catvehicle2 (yaml)
        self.turn_reward = rospy.get_param("/catvehicle2/turn_reward")
        self.end_episode_points = rospy.get_param("/catvehicle2/end_episode_points")

        self.cumulated_steps = 0.0

        self.laser_filtered_pub = rospy.Publisher('/catvehicle2/laser/scan_filtered', LaserScan, queue_size=1)
    ...
    def _get_obs(self):
        ...
        #laser_scan = self.get_laser_scan()
        get_dist = self.get_dist() #add
        get_angle = self.get_angle() #add
        ...
        rospy.logdebug("Observation >>>"+str(get_dist)+ ", "+str(get_angle))
        rospy.logdebug("AFTER DISCRET_episode_done==>"+str(self._episode_done))
        rospy.logdebug("END Get Observation ==>")
        return [get_dist, get_angle]
